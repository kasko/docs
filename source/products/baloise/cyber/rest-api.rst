...
.. csv-table::
   ...
   "duration", "yes", "``string``", "The policy duration in years, accepted values are ``PY1`` for ``single year``, ``PY2`` for ``two years`` and ``PY3`` for ``three years``."
...
.. code:: bash
    ...
    curl https://api.kasko.io/quotes \
        ...
        -d item_id=item_c86957fb952733fcadecaa75d9f \
        -d language=de \
        -d data='{"duration":"PY1","product":"3","family":"2"}'
...
.. csv-table::
   ...
   "gender",      "yes", "``string``", "Customers gender, accepted values are ``mr`` or ``ms``."
